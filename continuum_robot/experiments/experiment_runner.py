...
class ExperimentRunner:
    ...
    def run(
        ...
        for point in points:
            for repeat_index in range(max(1, point.repeat)):
                ...
                tool_0a = self.tracker_manager.get_latest_tool("0A")
                tool_0b = self.tracker_manager.get_latest_tool("0B")
                if tool_0a is None:
    ...
    def _build_row(
        ...
        repeat_index: int,
        tool_0a: TrackerToolState | None,
        tool_0b: TrackerToolState | None,
        ...
        if tool_0a is not None and tool_0a.valid:
        ...
        def _tool_translation(tool: TrackerToolState | None) -> list[float]:
            if tool is None:
