...
class SystemTab(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self.servo_profile_speed_spin = QSpinBox()
        self.servo_profile_speed_spin.setRange(1, 300)
        self.servo_profile_speed_spin.setValue(3)
        self.servo_profile_speed_spin.valueChanged.connect(self._mark_parameter_dirty)
        ...
        self.parameters_form.addRow("Servo speed", self.servo_profile_speed_spin)
        self.parameters_form.addRow("Speed status", self.servo_profile_status_label)
    ...
    def _save_runtime_parameters(self) -> None:
        parameters = {
            ...
            "servo_profile_speed": int(self.servo_profile_speed_spin.value()),
        }
    ...
    def _apply_parameter_values(self, state: SystemViewState, values: dict[str, object]) -> None:
        ...
        try:
            ...
            set_spinbox_value(self.servo_profile_speed_spin, int(values["servo_profile_speed"]), block_signals=True)
            self._sync_operating_mode_visibility()
        finally:
            self._updating_parameter_widgets = False
        self._sync_operating_mode_visibility()
    ...
    def _parameter_values_from_state(self, state: SystemViewState) -> dict[str, object]:
        return {
            ...
            "servo_profile_speed": int(state.servo_profile_speed),
        }
